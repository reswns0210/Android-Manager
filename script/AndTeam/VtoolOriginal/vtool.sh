...
then
clear
echo "Android Manager 1.0 Script"
echo ""
echo "계정상태: Beta`getprop ro.build.type`"
echo "현재기기:`getprop ro.product.model`"
echo "현재버젼:`getprop ro.product.software_ver`(펌웨어버젼)"
echo "현재버젼:`getprop ro.build.version.release`(안드로이드 버젼)"
echo "   SDK  :`getprop ro.build.version.sdk`"
echo "제조업체:`getprop ro.product.manufacturer`"
echo "메인보드:`getprop ro.product.board`"
echo "Build KST(GMT+9) 13:16 2016/5/27"
echo ""
echo "------------------------------------"
echo "|    기능               번호       |"
echo "| 루팅카운트 초기화      1         |"
echo "| 준비중입니다.(정식버젼 업데이트) |"
echo "| 파티션 백업            3         |"
echo "| 파티션 복원            4         |"
echo "| zip파일 플래싱         5         |"
echo "| WIFI 맥주소 변경       6         |"
...
echo "인해 발생한 문제는 VTEAM은 책임지지 않습니다."
echo ""
else
  clear
  echo "이 툴을 사용하기 위해서는 루팅이 되어있어야 합니다. 루팅을 한뒤 실행해주세요."
fi
...
echo "ui_print(\"vtool mobile installer\");" >> /cache/recovery/extendedcommand
...
then
echo "print ""vtool mobile installer" > /cache/recovery/openrecoveryscript
 (if [ "$sdcardfind" = emulated/0 ];
then
...
echo ""
echo "exam) vtool 1"
echo "3번메뉴: vtool 3 파티션이름 백업할경로"
echo "exam) vtool 3 boot /sdcard/boot.img"
echo ""
echo "4번메뉴: vtool 4 파티션이름 백업했던 경로"
echo "exam) vtool 4 boot /sdcard/boot.img"
echo ""
echo "5번메뉴: vtool 5 리커버리종류 옵션 옵션 옵션"
...
echo ""
echo "6번메뉴:vtool 6 xxxxxxxxxxxx"
...
echo "맥주소 변경후 원래의 맥주소를 잊어버렸을 경우 vtool 개발자는 책임지지 않습니다."
echo ""
echo "7번메뉴: vtool 7 모드"
echo "모드 종류"
echo "1: 재부팅"
echo "2: 리커버리모드 진입"
echo "3: 시스템 종료"
echo "4: 빠른 재부팅"
echo "8번메뉴 vtool 8  옵션 옵션 옵션"
...
echo "print ""vtool mobile multirom flashing" > /cache/recovery/openrecoveryscript
...
clear
  ...
  echo "VERSION INFO"
  echo ""
  echo "V Tool Build 22338"
  echo "Build KST(GMT+9)13:16 2016/5/27"
  echo "이 스크립트는 V TEAM 내부 테스트용으로 제작되었습니다"
  echo "무단 유출을 금합니다"
  echo ""
elif [ "$1" -eq 777 ]; then    
clear
...
# 이 스크립트의 저작권은 VTeam이 있으며 무단 수정 · 2차 수정후 배포을 금합니다. 